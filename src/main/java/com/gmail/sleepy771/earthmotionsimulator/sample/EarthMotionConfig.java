...
import com.gmail.sleepy771.earthmotionsimulator.simulation.impl.MultiThreadedExecutor;
...
public class EarthMotionConfig extends AbstractRunConfiguration implements RunConfiguration {
	...
	private SolarSystemSimulationFactory factory;
	...
	public void beforeStart() {
		...
		bodies.add(converter.convertSystemUnit(new Planet(new BodyID("Sun", 0),  1.988500E30, new double[] {1.142040439574852E-03, 5.472638374274859E-05, -1.391470641511653E-04}, new double[] {2.018456216623680E-04, -4.644973878159391E-03, -1.702282443840230E-04})));
		...
		bodies.add(converter.convertSystemUnit(new Planet(observedObject = new BodyID("Earth", 3), 5.97219E24, new double[] {-6.763168158278883E-03, 1.541758230939028E-02, -1.401410798416276E-04}, new double[] {8.929094222092794E-01, 4.368856823025477E-01, -1.851312314686687E-04})));
...
//		releasAll();
	...
	protected ParticleSystem createSystem() {
		return new ParticleSystem(bodies, date, 60000);
	}
	...
	@Override
	protected SimulationExecutor createExecutor() {
		return new MultiThreadedExecutor();
	}

	@Override
	protected SimulationCondition createRunCondition() {
		return new SolarSystemSimualtionEndCondtion(((ParticleSystem) getSystem()), new DateTime(2020, 9, 14, 0, 0));
	}
	...
	@Override
	protected SimulationFactory getFactory() {
		if (factory == null) {
			factory = new SolarSystemSimulationFactory((ParticleSystem) getSystem(), this);
		}
		return factory;
	}

	@Override
	protected List<Simulation> listSimulations() {
